...
void DS3231::setTime(int hour, int min, int sec)
{
...
void DS3231::setDate(int date, int mon, int year)
{
...
void DS3231::setDOW(int dow)
{
	if ((dow>0) && (dow<8))
		_writeRegister(REG_DOW, dow);
}

char *DS3231::getTimeStr(int format)
{
...
char *DS3231::getDateStr(int slformat, int eformat, char divider)
{
...
char *DS3231::getDOWStr(int format)
{
...
char *DS3231::getMonthStr(int format)
{
...
{
  int _reg = _readRegister(REG_STATUS);
  _reg &= ~(1 << 3);
  _reg |= (enable << 3);
...
{
  int _reg = _readRegister(REG_CON);
  _reg &= ~(1 << 2);
  _reg |= (enable << 2);
...
{
	int _msb = _readRegister(REG_TEMPM);
	int _lsb = _readRegister(REG_TEMPL);
...
void DS3231::_writeByte(int value)
{
	pinMode(_sda_pin, OUTPUT);
	shiftOut(_sda_pin, _scl_pin, MSBFIRST, value);
}

int	DS3231::_decode(int value)
{
	int decoded = value & 127;
	decoded = (decoded & 15) + 10 * ((decoded & (15 << 4)) >> 4);
	return decoded;
}

int DS3231::_decodeH(int value)
{
  if (value & 128)
...
int	DS3231::_decodeY(int value)
{
	int decoded = (value & 15) + 10 * ((value & (15 << 4)) >> 4);
	return decoded;
}

int DS3231::_encode(int value)
{
	int encoded = ((value / 10) << 4) + (value % 10);
	return encoded;
}
